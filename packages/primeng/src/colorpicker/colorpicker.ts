    ...
    template: `
        <button
            *ngIf="!inline"
            ...
            type="button"
            readonly
            [class]="cn(cx('preview'), inputStyleClass)"
            ...
            <ng-template #content>
                <div [class]="cx('panel')" [pBind]="ptm('panel')">
                    <div [class]="cx('content')" [pBind]="ptm('content')">
                        ...
                        <input #inputValue type="text" [class]="cx('inputValue')" [ngClass]="{ 'p-disabled': $disabled() }" (input)="onInputValueChange($event)" [pBind]="ptm('inputValue')" />
                    </div>
                </div>
            </ng-template>
        </p-overlay>
...
export class ColorPicker extends BaseEditableHolder<ColorPickerPassThrough> implements AfterViewChecked {
    ...
    /**
     * Style class of the preview element.
     * @group Props
     */
    ...
    /**
     * Preview-only color seed applied without going through the model.
     * @group Props
     */
    ...
    onInputValueChange(event: Event) {
        ...
        if (!this.inputValueRegex.test(stringValue)) {
            return;
        }
    ...
    writeControlValue(value: any): void {
        if (value) {
            switch (this.format) {
                ...
                case 'hsb':
                    this.value = value;
                    const rgb = this.HSBtoRGB(value);
                    this.inputBgColor = `rgb(${rgb.r}, ${rgb.g}, ${rgb.b})`;
                    break;
            }
        } else {
