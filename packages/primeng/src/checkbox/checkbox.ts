...
@Component({
    ...
    template: `
        ...
        <label *ngIf="label" [class]="labelStyleClass" [ngClass]="{ 'p-checkbox-label': true, 'p-checkbox-label-active': checked, 'p-disabled': $disabled() }" [attr.for]="inputId">{{ label }}</label>
...
export class Checkbox extends BaseEditableHolder<CheckboxPassThrough> {
    ...
    /**
     * Trailing icon class rendered after the label.
     * @group Props
     */
